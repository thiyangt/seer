...
#' @export
sim_arimabased <- function(y, Nsim, Combine=TRUE, M=TRUE, Future=FALSE, Length=NA, extralength=NA, seed_value){
    if (M ==TRUE){
      if ("Combine"==TRUE){
      train <- y$x
      test <-  y$xx
  ...
  mat <- list()
  for(i in 1:Nsim){
    mat[[i]] <- simulate(fit, nsim=length_series, future=Future, seed=seed_value)}
  return (mat)
}
#'@examples
#'require(Mcomp)
#'quaterly_m3 <- subset(M3, "yearly")
#'sim_arimabased(quarterly_m3[[1]], 2, Combine=TRUE, M=TRUE, Future=FALSE, seed_value=8)
#'
#'set.seed(1)
#'tsy <- ts(rnorm(8), frequency=1)
#'sim_arimabased(tsy, 2, Combine=FALSE, M=FALSE, Future=TRUE, Length=5, seed_value=8)
